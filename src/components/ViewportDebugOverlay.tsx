...
export default function ViewportDebugOverlay() {
  ...
  useEffect(() => {
    const update = (source: string) => {
      ...
      setMetrics(m);
      setLog((prev) => {
        const line = `${m.t} [${source}] vv=${m.vvW}x${m.vvH}@${m.vvOffX},${m.vvOffY} inner=${m.innerW}x${m.innerH} body=${m.bodyH}(${m.bodyTop}..${m.bodyBottom}) nav=${m.navTop}..${m.navBottom}`;
        return [...prev.slice(-14), line];
      });
    };
